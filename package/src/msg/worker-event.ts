...
import { isErrorEventInit, isMessageEventInit } from "../util/event-init.js";

interface WorkerEventInitMap
	extends Required<Record<WorkerEventType, EventInit>> {
...
export const isWorkerEvent = (message: unknown): message is WorkerEvent => {
	if (Array.isArray(message) && message.length === 2) {
		...
		return (
			typeof event === "string" && typeof init === "object" && init != null
		);
	}
	return false;
};
...
export const hasValidWorkerEventInit = <T extends WorkerEventType>(
	...
	switch (event) {
		case "error":
			return isErrorEventInit(init);
		case "message":
		case "messageerror":
			return isMessageEventInit(init);
		default:
			return false;
	}
};
